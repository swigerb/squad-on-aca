...
function validateManifest(manifest) {
  ...
  // Required external services are explicitly unsupported. The worker will not
  // open network egress or shell out to probe reachability (doing so would
  // expand the security/network surface), so a service a task genuinely cannot
  // run without cannot be validated here. Declaring one `required: true` would
  // therefore always fail preflight, so it is rejected at validation with an
  // actionable message instead of being a guaranteed, confusing runtime block.
  // Services declared `required: false` remain advisory (surfaced, never
  // blocking) so a manifest can still document service dependencies.
  if (Array.isArray(manifest.services)) {
    manifest.services.forEach((item, idx) => {
      if (isPlainObject(item) && item.required === true) {
        errors.push(
          `"services[${idx}]".required must be false: required external services are not supported because the worker cannot validate service reachability without expanding network egress. Declare the service with required: false (advisory) or provision and verify it out of band.`
        );
      }
    });
  }

...
assert_contains "$out" "Missing required tool: definitely-not-installed-binary" "missing required tool: names the tool"
...
unset SQUAD_TEST_FIXTURE_MISSING_TOKEN 2>/dev/null || true
out="$(bash "$PREFLIGHT" "$repo" 2>&1)"
rc=$?
assert_eq "78" "$rc" "missing required credential: exits 78"
assert_contains "$out" "Missing required credential: SQUAD_TEST_FIXTURE_MISSING_TOKEN" "missing required credential: names the credential"
rm -rf "$repo"

# 5. Same manifest, credential now provided -> passes.
repo="$(make_repo)"
cp "${FIXTURES}/missing-required-credential.yml" "${repo}/squad-capabilities.yml"
out="$(SQUAD_TEST_FIXTURE_MISSING_TOKEN=set-for-test bash "$PREFLIGHT" "$repo" 2>&1)"
rc=$?
